...
from layers import GraphVae, MLP, RecursiveGraphConvolutionStepAddOn
from loss import reconstruction_loss, vae_loss, recursive_loss_with_noise
...
parser.add_argument('--size_update_ratio', type=float, default=0.33)
parser.add_argument('--weight_decay', type=float, default=0.0)
args = parser.parse_args()

size_update_ratio = args.size_update_ratio
hidden_dim = args.hidden_dim
out_dim = args.out_dim
cite_data = args.data_set
weight_decay = args.weight_decay
dataset_str = args.data_set
norm=None
...
############ prepare data ##############
adj, feat = load_data(dataset_str)
...
optimizer = torch.optim.Adam(gcn_step.parameters(), lr=1e-3, weight_decay=weight_decay)
...
for batch_idx in range(num_iters):
    ...
    # train r-gcn
    adj_copy = adj_label.numpy().copy()
    X_copy = feat.numpy().copy()
    ...
    adj_copy = torch.from_numpy(adj_copy.astype(np.float32))
    X_copy = torch.from_numpy(X_copy.astype(np.float32))
    optimizer.zero_grad()
    gcn_step.train()
    loss = recursive_loss_with_noise(gcn_step, adj_copy, X_copy, size_update, norm)
    loss.backward()
    optimizer.step()
    ...
    print('R-GCN [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(batch_idx, num_iters,
            100. * batch_idx / num_iters,
            loss.item()))
    ...
    if batch_idx % 10 == 0:
        ...
        with torch.no_grad():
            ...
            # test r-gcn
            gcn_step.eval()
            z_mean, z_log_std = gcn_step(adj_label, feat)
            normal = torch.distributions.Normal(0, 1)
            z = normal.sample(z_mean.size())
            z = z * torch.exp(z_log_std) + z_mean

            roc, ap = get_roc_score(val_edges, val_edges_false, z.numpy())
            print('R-GCN val AP: {:.6f}'.format(ap))
            print('R-GCN val AUC: {:.6f}'.format(roc))
...
with torch.no_grad():
    ...
    roc, ap = get_roc_score(test_edges, test_edges_false, z.numpy())
    print('R-GCN test AP: {:.6f}'.format(ap))
    print('R-GCN test AUC: {:.6f}'.format(roc))
